...
(The filename is `DESIGN.md` — lowercase `.md`. Earlier revisions of this file said `DESIGN.MD`,
which resolves only because macOS is case-insensitive.)

⚠️ **`DESIGN.md` opens with a `UI binding rules` section that outranks its own tokens.** Those are
product rules — what the interface may and may not show (no confidence, no accuracy percentage, no
point value on the card, no rank numerals, macros never behind a tap). Read them before any UI work;
a token can be renegotiated, those were each ruled against a plausible alternative.

That section is a **sync pair** with `menu-scan-kb/docs/design-system/ui-binding-rules.md`. Change
one, change the other in the same session, then run `python3 ../menu-scan-kb/check-sync.py`.
...
For any UI Task:
- Use the provided design system that is in the `DESIGN.md` file, **starting with its
  `UI binding rules` section** — those constrain what may be shown at all, before any styling question.
