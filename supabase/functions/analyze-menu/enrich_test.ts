...
Deno.test("B25: ethanol reaches the calorie total at 7 kcal/g, and nothing else", () => {
  const ing = (
    name: string,
    typical_serving_g: number,
    per: Partial<Record<"protein_per_100g" | "carb_per_100g" | "fat_per_100g" | "alcohol_per_100g", number>>,
  ) => ({
    name,
  ...
  // The measured case: 150 ml of red wine returned 18 kcal against a real ~125.
  // Wine is ~10.6 g of ethanol per 100 g plus ~3 g of carbohydrate.
  const wine = sumIngredientMacros(
    [ing("red wine", 150, { carb_per_100g: 3, alcohol_per_100g: 10.6 })],
    null,
  );
  // 4*4.5 + 7*15.9 = 18 + 111.3
  assertEquals(wine.estimated_calories, 129);
  // Alcohol must NOT leak into a macro a diner filters on.
  assertEquals(wine.protein_g, 0);
  assertEquals(wine.fat_g, 0);
  assertEquals(wine.carb_g, 5);

  // Food is untouched: absent or zero alcohol must reproduce plain Atwater.
  const food = sumIngredientMacros(
    [ing("chicken", 100, { protein_per_100g: 31, fat_per_100g: 3.6 })],
    null,
  );
  assertEquals(food.estimated_calories, Math.round(4 * 31 + 9 * 3.6));

  // An archived pre-B25 ingredient has no alcohol field at all and must not NaN.
  const legacy = sumIngredientMacros(
    [{
      name: "x",
  ...
  assertEquals(legacy.estimated_calories, 170);
});
