...
if (!dumpPath || !ocrPath) {
  console.error("usage: replay-colocation.ts <dump.json> <ocr-cache.json> [photo.png]");
  Deno.exit(2);
}

let ocrRaw: { pages?: { blocks?: OcrBlock[] }[] };
try {
  ocrRaw = JSON.parse(await Deno.readTextFile(ocrPath));
  console.log(`OCR cache hit: ${ocrPath} ($0)`);
} catch {
  if (!photoPath) throw new Error(`no cache at ${ocrPath} and no photo given`);
  const apiKey = Deno.env.get("MISTRAL_API_KEY");
  if (!apiKey) throw new Error("MISTRAL_API_KEY required for the one paid OCR call");
  ...
  const res = await fetch("https://api.mistral.ai/v1/ocr", {
    ...
    body: JSON.stringify({
      ...
      document: { type: "image_url", image_url: `data:image/png;base64,${btoa(b64)}` },
      include_blocks: true,
    }),
  });
  if (!res.ok) throw new Error(`Mistral OCR ${res.status}: ${(await res.text()).slice(0, 500)}`);
...
const blocks = toBlockTexts((ocrRaw.pages ?? []).flatMap((p) => p.blocks ?? []));
const dump = JSON.parse(await Deno.readTextFile(dumpPath)) as { items?: ExtractedMenuItem[] };
if (!Array.isArray(dump.items)) throw new Error(`dump has no items array: ${dumpPath}`);
...
for (const d of dropped) console.log(`DROPPED: ${d.name} $${d.price} [${d.section_title}]`);
