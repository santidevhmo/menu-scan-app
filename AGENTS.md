...
**Current architecture decision (ruling 30) — ✅ PORTED INTO THE EDGE 2026-08-01 (C3), ⛔ STILL NOT DEPLOYED:** the edge function now RUNS this split (`ocrMistral` → `structureMenuText` in `supabase/functions/analyze-menu/`), with the OCR model PINNED to `mistral-ocr-4-0` (ruling 35) and the structuring model pinned to `gpt-4.1-2025-04-14`. The DEPLOYED function still runs the OLD path — deploying is a separate explicit step, and it is BLOCKED: the first 9-menu ×3 live gate (C4) failed 0/3 and exposed that **the model returns a different-but-valid item list per call**, so quality must be quoted as a RANGE across draws (33-37/45), never as a single number. One release-blocking defect is open — a menu silently returning 36 of 48 dishes on roughly 1 run in 3. See the horizontal-menus DELEGATION-BRIEF progress log and master-roadmap lessons 24-27 before touching extraction.
