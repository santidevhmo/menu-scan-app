...
> **✅ CURRENT BEST: `macro-best-v8` (B21 + B24b), and it IS DEPLOYED — edge function v29 since
> 2026-08-09 15:46 MST, verified against the live function on 2026-08-11.** ⚠️ Every "v28 / not
> deployed" line further down predates that deploy and is wrong; the deployed `ENRICH_PROMPT` is
> byte-identical to this repo's. **Check the live function, never a doc** — see START-HERE for how.
> Live figures on the 8-dish set, 4 runs x 3 draws:
...
29. **WHAT IS DEPLOYED IS A FACT ABOUT THE SERVER, NOT ABOUT A DOCUMENT — go and look.** For two days every status doc in this repo (START-HERE, this roadmap, the product plan) stated "production runs B4 as edge function v28; `macro-best-v8` is NOT deployed". Production had been running v29 — `macro-best-v8` — since 2026-08-09 15:46 MST, 35 minutes after the last enrichment commit. Nobody lied: the deploy simply happened after the sentences were written, and a claim about the past does not update itself. The cost is asymmetric and one-directional — a session that believes the newest work is unshipped will re-measure it, re-argue whether to ship it, or "fix" a defect the live prompt no longer has. **The check is two minutes and $0:** `mcp__supabase__list_edge_functions` for the version and `updated_at`, `mcp__supabase__get_edge_function` for the source, then test whether the deployed bundle contains this repo's `ENRICH_PROMPT` **verbatim** — that string is the version fingerprint, and it is the only comparison that cannot be fooled by a stale comment or a similar-looking diff. (The payload JSON-escapes it, so escape quotes and newlines before comparing.) **Do this before quoting any deployment state, and before designing anything whose value depends on what is live.** Corollary: when you deploy, edit the docs in the SAME commit — the 35-minute gap is exactly how this happened.
