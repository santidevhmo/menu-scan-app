...
- **Working directory:** the worktree `/private/tmp/menu-scan-app-extraction-eval-harness` on branch `feat/extraction-eval-harness`. All code/harness/ledger commands run there. Plan/roadmap/diagram edits happen in the main checkout (`/Users/santiagoaguirre/Desktop/CODING/menu-scan-app`). If the worktree path is gone, recreate it: `git worktree add /private/tmp/menu-scan-app-extraction-eval-harness feat/extraction-eval-harness`.
