...
**Working set that must not regress (all TDD'd, tile-pass-scoped):** tolerated_categories (scorer), dropBannerEchoOptions, nullPriceNoteSections, twin fold + truncation drop (merge.ts), parseItemGrams comma fix, misread pin substrings ("tiBoneless", "Megach", Cabeza/Cubeta + "El Tender" unchecked keys).
