...
4. **Horizontal/landscape menu handling** (user 2026-07-12) — detect menu orientation and extract accordingly. Expected user behavior: a physically landscape menu gets photographed in portrait, rotated 90° (menu's left edge at the top, right edge at the bottom — as if the menu in front of you were flipped 90°). Candidate approach: detect the rotation (EXIF, aspect ratio + a cheap model signal, or P1's layout assessment) and rotate the image upright client-side before phase-1; tiles then cut from the rotated image. Never tested — all 6 fixtures are portrait-upright; needs at least one landscape fixture (can reuse an existing menu photographed/rotated sideways) + detector false-positive assertions on the 6 upright menus (same discipline as the dense detector). **ACTIVE since 2026-07-13 as the containerized sub-roadmap `docs/superpowers/horizontal-menus/` on the eval-worktree branch (entry point: its DELEGATION-BRIEF.md; rotation is its Phase 3). Status 2026-07-21: RESTRUCTURED into launch plan H1–H4 (container ruling 27; horizontal ships WITH launch incl. rotation). **H1 recall-safe tiling CLOSED 2026-07-21 (eval-093 gate PASS: all 3 wide menus recall-clean ×3 through the landscape 0.65/0.35 tile geometry — polloteria dense all-5-dims, bistro all-5-dims, guest-house full 48/48 incl. all 12 Enhancements + 3 Sides, zero inventions; fixes = landscape gridCropRects overlap + existence-tier merged-row split + null-grams twin-fold + section-title scorer tolerances).** 1f dense-vs-not detection = PARKED post-release cost-opt (ruling 24; evals 086–088 closed cheap discrimination). Semantic tile verifiers + nested geometry exhausted/rejected (evals 060–072) — never re-propose. **NEXT = H2 rotation** (sideways camera captures → upright; content-based + direction-aware, rulings 22–23; NOT built) → H3 production wiring (tile trigger INERT until client sends photo_dims) → H4 combined 9-menu gate + device. Entry point: container DELEGATION-BRIEF.md newest PROGRESS LOG entry; Phase-2 eval set (GH Shape-A + polloteria Ensaladas) in container ROADMAP MANDATORY RESTORE.**
